...
// POST /api/exam/translate — translate questions to Hindi via Anthropic API (server-side, key safe)
router.post('/translate', auth, async (req, res) => {
  ...
  const payload = questions.map((q, i) => ({ i, q: q.question, opts: q.options }));

  const prompt = `Translate the following exam questions and their options from English to Hindi.
Return ONLY a valid JSON array, no extra text, no markdown fences.
Each item must have: i (original index number), q (translated question string), opts (array of exactly 4 translated option strings).
Input:
